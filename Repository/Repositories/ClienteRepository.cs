...
namespace Repository.Repositories
{
    public class ClienteRepository : ICliente
    {
        ...
        {
            var cliente = _contexto.Clientes.Include(cliente => cliente.Facturas).FirstOrDefault(cliente => cliente.Id == id && cliente.Estado == "ACTIVO");
            if (cliente == null)
            {
        ...
        {
            var clientes = _contexto.Clientes.Include(cliente => cliente.Facturas).Where(cliente => cliente.Estado == "ACTIVO").ToList();
            if (clientes == null)
            {
                throw new Exception("No se encontró el cliente con el id solicitado");
            }
        ...
        {
            ClienteModel? resultado = _contexto.Clientes.FirstOrDefault(c => c.Documento == documento);
